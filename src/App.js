...
class App extends Component {
  ...
  onClickItem = id => {
    ...
    const filterThumbnailImage = imagesList.filter(
      eachItem => eachItem.id === id,
    )
    if (filterThumbnailImage[0].imageUrl === matchImage) {
      this.setState(prevState => ({score: prevState.score + 1}))
    } else {
      this.clearInterval()
      this.setState({loose: true})
    }
    const randomIndexValue = Math.ceil(Math.random() * 29)
    this.setState({matchImage: imagesList[randomIndexValue].imageUrl})
  }
  ...
  render() {
    const {activeTabId, matchImage, score, loose, timer} = this.state
    ...
    return (
      <div className="divContainer">
        <NavHeader scoreValue={score} timerValue={timer} />
        {loose === true ? (
